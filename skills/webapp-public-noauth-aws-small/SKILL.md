...
#### セキュリティグループ
以下の構成（Postgres DBを想定）を基本とし、必要に応じてポートやルールを変更
```
[sg-alb]
...
[sg-ecs]
  Inbound:  sg-alb → 8000
            sg-ecs → 443 (SSM用)
            sg-maintenance → 443 (SSM用)
  Outbound: sg-rds → 5432
            0.0.0.0/0 → 443 (ECR pull, S3, CloudWatch, SSM用)
[sg-rds]
  Inbound:  sg-ecs → 5432
...
[sg-maintenance]
  Inbound:  なし
  Outbound: sg-rds → 5432
            Anywhere-IPv4 → 443 (SSM, 用)
```

#### VPCエンドポイント
以下の構成を基本とし、必要に応じてサービスやルートテーブルを変更
```
[vpce-s3]
  Service: com.amazonaws.ap-northeast-1.s3
  Type: Gateway
  Route Table: Public Subnet 1, Public Subnet 2
[vpce-dkr]
  Service: com.amazonaws.ap-northeast-1.ecr.dkr
  Type: Interface
  Route Table: Private Subnet 1, Private Subnet 2
...
[vpce-ssm]
  ...
  Security Group: sg-ecs
```
なお、vpce-logsはECSタスクログのCloudWatch Logsへの送信、vpce-ssmはDB接続情報等のシークレット管理を主目的としている
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::{project-name}-data-{account-id}-ap-northeast-1-an",
        "arn:aws:s3:::{project-name}-data-{account-id}-ap-northeast-1-an/*"
      ]
    }
  ]
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::{project-name}-data-{account-id}-ap-northeast-1-an/*"
    }
  ]
}
```
...
- Master username: {project-name}-migrator（マイグレーション使用を想定）
...
- Initial database name: プロジェクトに応じて適宜設定（例: {project-name}_db）
...
CREATE USER {project-name}_app WITH PASSWORD '（強力なパスワードを設定）';

-- 02-init.sql 2) スキーマの所有権と権限設計
ALTER SCHEMA public OWNER TO {project-name}_migrator;
GRANT CONNECT ON DATABASE {project-name}_viewer TO {project-name}_app;
GRANT USAGE ON SCHEMA public TO {project-name}_app;
GRANT USAGE, CREATE ON SCHEMA public TO {project-name}_migrator;

-- 3) 既存テーブルへの権限（マイグレーション後に実行）
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO {project-name}_app;
GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA public TO {project-name}_app;

-- 4) 今後作成されるテーブルへの権限も自動付与
ALTER DEFAULT PRIVILEGES FOR ROLE {project-name}_migrator IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO {project-name}_app;
ALTER DEFAULT PRIVILEGES FOR ROLE {project-name}_migrator IN SCHEMA public
  GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO {project-name}_app;
...
- Type: String
- Value: 上でRDSインスタンスのInitial database nameに設定した値（例: {project-name}_db）
4. RDSのマイグレーション用ユーザー名
- Name: /{project-name}/db/migrator_user
- Description: RDS username for migration
- Type: String
- Value: 上でRDSインスタンスのMaster usernameに設定した値（例: {project-name}_migrator）
...
- Type: String
- Value: 上で作成したアプリ用ユーザー名（例: {project-name}_app）
7. RDSのアプリ用ユーザーパスワード
...
- Type: String
- Value: {project-name}-data-{account-id}-ap-northeast-1-an（下で作成するデータバケット名）
9. Reactの静的ファイルを保存するバケット名
- Name: /{project-name}/s3/static_bucket
- Description: S3 bucket name for React static files
- Type: String
- Value: {project-name}-static-{account-id}-ap-northeast-1-an（下で作成する静的ファイルバケット名）

### S3 bucket
以下2種類のバケットを作成
1. `{project-name}-data-{account-id}-ap-northeast-1-an`: 配信する画像等のデータを保存するバケット（非公開）
- Bucket name prefix: `{project-name}-data`

2. `{project-name}-static-{account-id}-ap-northeast-1-an`: Reactの静的ファイル（CloudFront経由で公開）
...
アプリにCloudFrontドメイン（`*************.cloudfront.net`）でアクセスできるようにするため、以下の構成でCloudFrontディストリビューションを作成する（index.htmlはキャッシュなしで、その他の静的ファイルはキャッシュありでS3の`{project-name}-static-{account-id}-ap-northeast-1-an`バケットに設置済の前提）
- Distribution name: {project-name}-frontend
- Distribution type: Single website or app
- Origins
  - Origin1（フロントエンド用）
    - Origin domain: {project-name}-static-{account-id}-ap-northeast-1-an.s3.ap-northeast-1.amazonaws.com（静的ファイルをアップロードしたバケット）
...
S3バケットにもCloudFront OACからのアクセスを許可するポリシーを追加する
```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::{project-name}-static-{account-id}-ap-northeast-1-an/*",
      "Condition": {
        "ArnLike": {
